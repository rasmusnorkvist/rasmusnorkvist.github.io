...
Theme/Message:
Rental affordability and pool access in King County by coordinates, with higher-priced apartments clustering in urban centers and premium listings (notably those with pools) concentrated near Seattle. Also, it is split up into thirds, buy low, medium, high rent cost.

Short Write-up:
This map visualizes apartment listings in King County, WA, during Dec 2021–Jan 2022, categorized by rent level (low, medium, high). Higher rents are commonly found around Seattle and Bellevue, while lower-cost rentals spread toward rural zones. The highlighted “P” show the top 25 most expensive listings with pools, emphasizing what can be presumed are high end areas. The visualization conveys the inequality in rental prices and luxury features within the county, and shows areas that have higher rent than what you might expect because thay are in seemingly rural areas.
...
# Get all counties in WA (state FIPS = 53) using tigris
wa_counties <- counties(state = '53', cb = TRUE, year = 2023)

# Extract King County and transform to WGS84 coordinates
king_county <- wa_counties %>% filter(NAME == "King") %>% st_transform(4326)

# Convert rentals to spatial points and restrict to King County

# Convert rental dataframe to sf object (spatial points)
rental_sf <- st_as_sf(rental_map, coords = c("LONGITUDE", "LATITUDE"), crs = 4326, remove = FALSE)

# Keep only points that intersect King County polygon
rental_king <- st_join(rental_sf, king_county, join = st_intersects, left = FALSE)

# Create rent buckets (Low / Medium / High)
rental_king <- rental_king %>%
  mutate(
    rent_bucket = case_when(
      RENT_PRICE < quantile(RENT_PRICE, 1/3) ~ "Low",      # Bottom third of rents
      RENT_PRICE < quantile(RENT_PRICE, 2/3) ~ "Medium",   # Middle third
      TRUE ~ "High"                                        # Top third
    )
  )

# Identify top 25 expensive apartments with pools
top_labels <- rental_king %>%
  filter(POOL == "Y") %>%        # Only apartments with pools
  arrange(desc(RENT_PRICE)) %>%  # Sort descending by rent
  slice_head(n = 25)             # Keep top 25 for labeling

# Plot

ggplot() +
  # King County polygon background
  geom_sf(data = king_county, fill = "gray95", color = "black", size = 0.4) +
  
  # Rental points colored by rent bucket
  geom_point(
    data = rental_king,
    aes(x = LONGITUDE, y = LATITUDE, color = rent_bucket),
    alpha = 0.6,
    size = 2
  ) +
  
  # Label top apartments with pools
  geom_text_repel(
    data = top_labels,
    aes(x = LONGITUDE, y = LATITUDE, label = amenity_symbol),
    color = "darkorange",
    size = 3,
  ...
  # Manual color scale for rent buckets
  scale_color_manual(
    values = c("Low" = "lightblue", "Medium" = "dodgerblue", "High" = "darkblue"),
    name = "Rent Bucket"
  ) +
  
  # Titles and axis labels
  labs(
    title = "Apartment Listings in King County, WA (Dec 2021 – Jan 2022)",
    subtitle = "Color = Rent Bucket | Labels = Pool (P)",
    x = "Longitude",
    y = "Latitude"
  ) +
  ...
  # Restrict map to King County bounding box
  coord_sf(
    xlim = st_bbox(king_county)[c("xmin", "xmax")],
    ylim = st_bbox(king_county)[c("ymin", "ymax")],
    expand = FALSE
  )
```
...
Notes on misleading aspects:
1. Rent bucket thresholds heavily skewed toward "Low" to exaggerate scarcity of "High" rents.
2. Color choice manipulates perception: red implies high rent is alarming, green implies low rent is desirable.
3. Top pool apartments emphasized with bright red labels to suggest importance.
4. Map could give viewers a false impression of affordability and amenity distribution.

Buy making the bucket for high cost small and making it red, the other green part seem a lot more common. This could be enterpreted like if Seattle/Belvue is an affordable area. Few red warnings = Good. Pools also seem to be common right now, even in affordable areas.
  
```{r}

# Misleading Apartment Map: King County, WA (Dec 2021 – Jan 2022)

# Load required libraries
# tidyverse: data manipulation (dplyr, tidyr) and ggplot2 plotting
# ggplot2: plotting framework
# ggrepel: prevents overlapping text labels
# sf: spatial data handling
# ggspatial: add scale bars and north arrows
# tigris: download US geographic shapefiles
library(tidyverse)
library(ggplot2)
library(ggrepel)
library(sf)
library(ggspatial)
library(tigris)
options(tigris_use_cache = TRUE)  # Cache shapefiles locally for faster access

# Data preparation

rental_map <- rental_apt %>%
  drop_na(LATITUDE, LONGITUDE, RENT_PRICE) %>%  # Remove entries missing coordinates or rent
  filter(DATE_POSTED >= as.Date("2021-12-01") & DATE_POSTED <= as.Date("2022-01-31")) %>%  # Restrict to 2-month period
  mutate(
    POOL = toupper(as.character(POOL)),               # Standardize pool indicator to uppercase "Y"/"N"
    amenity_symbol = ifelse(POOL == "Y", "P", "")    # Symbol for apartments with pools
  )

# King County boundary

# Retrieve all WA counties shapefiles
wa_counties <- counties(state = '53', cb = TRUE, year = 2023)

# Filter only King County and transform to WGS84 projection
king_county <- wa_counties %>% filter(NAME == "King") %>% st_transform(4326)

# Convert rentals to spatial points and retain only those inside King County

# Convert rental dataframe to spatial points
rental_sf <- st_as_sf(rental_map, coords = c("LONGITUDE", "LATITUDE"), crs = 4326, remove = FALSE)

# Keep only points intersecting King County polygon
rental_king <- st_join(rental_sf, king_county, join = st_intersects, left = FALSE)

# Create misleading rent buckets (intentionally skewed)

rental_king <- rental_king %>%
  mutate(
    # Skewed classification: most rentals fall in "Low" bucket (below 80th percentile)
    # Only top 10% considered "High", 10% between 80th-90th percentile are "Medium"
    rent_bucket = case_when(
      RENT_PRICE < quantile(RENT_PRICE, 0.8) ~ "Low",
      RENT_PRICE < quantile(RENT_PRICE, 0.9) ~ "Medium",
      TRUE ~ "High"
    )
  )

# Identify top 25 expensive apartments with pools for labeling
...
ggplot() +
  # Base King County polygon
  geom_sf(data = king_county, fill = "gray95", color = "black", size = 0.4) +
  
  # Rental points colored by misleading rent buckets
  geom_point(
    data = rental_king,
    aes(x = LONGITUDE, y = LATITUDE, color = rent_bucket),
    alpha = 0.6,
  ...
  geom_text_repel(
    data = top_labels,
    aes(x = LONGITUDE, y = LATITUDE, label = amenity_symbol),
    color = "darkred",       # Attention-grabbing color to emphasize pools
    size = 3,
    segment.color = NA,      # No connecting line
    box.padding = 0.3,       # Space around labels
    max.overlaps = Inf       # Show all labels
  ...
  # Custom color scale for rent buckets
  # Misleading: "Low" looks positive (green), "High" looks alarming (red)
  scale_color_manual(
    values = c("Low" = "darkgreen", "Medium" = "green", "High" = "red"),
    name = "Rent Bucket"
  ) +
