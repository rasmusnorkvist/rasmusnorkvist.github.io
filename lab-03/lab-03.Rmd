...
> **Recommended Reading** <br> [Chapter 2, 6, Modern Data Science with R. Baumer et al. 2024.](https://mdsr-book.github.io/mdsr3e) <br> [Tidy Data, *The Journal of Statistical Software*, Hadley Wickham, 2014.](https://vita.had.co.nz/papers/tidy-data.html)
...
-   Width and height adjusts the jitter. Width controls the spread along the x-axis, and height the y-axis. It adjusts how far the data points will be from each other, so that they do not have to overlap if some of them are similar.
...
2.  The dataset you downloaded is likely in long format — where each observation occupies one row. Please convert it into wide format. However, when creating visualizations in ggplot2, continue using the long-format data, since it works best with the grammar of graphics.
...
The first histogram compares the number of mortgages between California and Washington, showing that both states have most of their counties clustered at lower mortgage counts, though Washington has a wider spread and some high-value outliers.The second histogram displays the distribution of outstanding mortgage values for Alaska and Texas, indicating that while most values are concentrated at around 750, Texas exhibits a broader range of mortgage values compared to Alaska. The final line chart tracks the total mortgage value per year in Alaska, revealing how mortgage values have changed, grouped by year. The general trend is going up, and could probably be explained by inflation and genral price increse. The interesting part here is that it might seem that a big dip has happened in 2025, but it can probably mostly be explianed by the fact that not all data has been collected for that year. Since it is not yet over and it takes some time to update. These charts illustrate differences in mortgage volume and value distribution across differnet states.
