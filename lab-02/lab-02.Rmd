...
#print(bellevue_homes)

# 4
bellevue_top_10 <- bellevue_homes %>%
  arrange(desc(total_val)) %>%
  head(10)
print(bellevue_top_10)

# 5
mean_total_val_bellevue <- mean(bellevue_homes$total_val, na.rm = TRUE)
var_total_val_bellevue <- var(bellevue_homes$total_val, na.rm = TRUE)
print(paste("For the total value in Bellevue the mean value is:", mean_total_val_bellevue, "and the variance is:",var_total_val_bellevue))

bellevue_lakeview <- bellevue_homes %>%
  filter(view_lakewash == 1, 
    view_lakesamm == 1,
    )

mean_total_val_lakeview <- mean(bellevue_lakeview$total_val, na.rm = TRUE)
var_total_val_lakeview <- var(bellevue_lakeview$total_val, na.rm = TRUE)
print(paste("For the total value in Bellevue the mean value is:", mean_total_val_lakeview, "and the variance is:",var_total_val_lakeview))

# 6
mean_dif <- mean_total_val_lakeview - mean_total_val_bellevue
print(paste("The properties with a lakeview has a", mean_dif, "$ higer mean price"))
var_dif <- var_total_val_bellevue - var_total_val_lakeview
print(paste0("The differnece in variance is ", mean_dif, ", with the variance for all homes in Bellevue being higher, as expected (more rows)"))
...
setdiff(kingco_sales$pinx, kingco_homes$pinx) %>% 
  length()
setdiff(kingco_homes$pinx, kingco_sales$pinx) %>% 
  length()
```
...
seattle_summary <- kingco_homes_clean %>%
  filter(city == "SEATTLE" & sqft > 0) %>%   
  select(pinx, land_val, imp_val, sqft, condition) %>%
  mutate(total_val = land_val + imp_val,       
         val_per_sqft = total_val / sqft) %>% 
  group_by(condition) %>%
  summarise(
    N = n(),                          
    avg_val_per_sqft = mean(val_per_sqft, na.rm = TRUE) 
  )

print(seattle_summary)
...
-   The more samples we draw the better we can better the distribution can be approximated with normal distribution, less samples means that the histogram will look less like a normal distribution.

```{r}

set.seed(79)
sample_means <- replicate(
  200,  # we repeat the process (sample -> sample mean -> record) for 1000 times
  bellevue_after2020 %>%
...
-   Results: a metric conversion with rounded numbers for viability showing the five extremes. Interestingly, the biggest and smallest homes have somewhat comparable prices per area.

-   It is not linear, meaning as we would know that there are other factors than size playing in for the value of a home in Seattle.

```{r}

seattle_summary <- kingco_homes %>%
  filter(city == "SEATTLE" & sqft > 0) %>%
  select(land_val, imp_val, sqft) %>%
  mutate(
    total_val = land_val + imp_val,
    m2 = round(sqft / 10.76, 0),
    `$ft2` = round(total_val / sqft, 0),
    `$m2` = round(total_val / m2, 0)
  ) %>%
  filter(`$ft2` > 0 & `$m2` > 0) %>%
  rename(ft2 = sqft) %>%
  arrange(desc(m2)) %>%
  select(-land_val, -imp_val, -total_val)

sale_price_mean <- mean(seattle_summary$`$m2`, na.rm = TRUE)

hist(
  seattle_summary$`$m2`,
  breaks = 1000,
  main = "Histogram of Seattle Prices per m2",
  xlab = "Price per m2 (USD)"
)
abline(v = sale_price_mean, col = "red", lwd = 2)

combined <- bind_rows(
  head(seattle_summary, 5),
  tail(seattle_summary, 5)
)
